...
def get_split_vert_for_loop(mesh, bm, l, flags):
    ...
    if len(bm.loops.layers.color) and flags & SECFLAGS_HAS_VERTEX_COLORS:
        ...
        if bake_layer:
            vcs = [ l[bake_layer][0], l[bake_layer][1], l[bake_layer][2], l[bake_layer][3] ]
        elif vc_layer:
            vcs = [ l[vc_layer][0], l[vc_layer][1], l[vc_layer][2], l[vc_layer][3] ]
        else:
            #print("No vertex color layer, using white!")
            vcs = [1.0, 1.0, 1.0, 1.0]
        alpha_layer = bm.loops.layers.color.get("alpha")
        vcs = (vcs[0], vcs[1], vcs[2], l[alpha_layer][0] if alpha_layer else vcs[3])
    else:
        vcs = None
